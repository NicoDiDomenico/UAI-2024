...
export const checkAuth = (req: Request, res: Response, next: NextFunction) => {
  ...
  try { /* Notar como no hace falta usar try catch con async await, esto es porque podemos tambien captar errores en funciones sincronicas */
    ...
    // Verificamos la validez del token JWT usando la función jwt.verify().
    // Si el token es válido, decodificamos su contenido (el payload).
    // Si el token no es válido (expirado, alterado, etc.), se lanzará un error.
    const decodedToken = jwt.verify(token, JWT_TOKEN) as IJwtPayload;
    // La aserción de tipo "as IJwtPayload" le dice a TypeScript que el resultado tiene el formato de la interfaz IJwtPayload.
    // Por ejemplo, el token podría contener propiedades como id, email, exp (fecha de expiración), etc.
