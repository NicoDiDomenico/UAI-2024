    ...
    intercept(context, next) {
        ...
        return next.handle().pipe((0, operators_1.tap)({
            next: () => {
                ...
                this.metricsService.endRequest(request.method, request.route?.path || request.path, response.statusCode, responseTime);
            },
            error: () => {
                const responseTime = Date.now() - startTime;
                this.metricsService.endRequest(request.method, request.route?.path || request.path, response.statusCode || 500, responseTime);
            },
        }));
    }
};
