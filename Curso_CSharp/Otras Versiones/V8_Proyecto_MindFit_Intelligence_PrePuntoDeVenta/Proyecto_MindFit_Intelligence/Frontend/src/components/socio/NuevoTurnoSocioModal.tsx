...
export function NuevoTurnoSocioModal({ onClose, onRegistered }: NuevoTurnoSocioModalProps) {
  ...
  const loadDisponibilidad = useCallback(async (nextFecha: string) => {
    ...
    try {
      ...
      setSelectedRangoId(getDefaultRangoId(activeRangos, nextFecha))
  ...
  async function handleSubmit() {
    ...
    try {
      await turnosService.registrarTurnoSocio({
        idUsuarioSocio: 0,
  ...
  return (
    <div className="consultar-backdrop consultar-backdrop--nested" role="presentation">
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="nuevo-turno-socio-title"
      >
        <header className="consultar-header nuevo-turno-modal__header">
          <div>
            <span className="section-kicker">Turnos / Nuevo</span>
            <h2 id="nuevo-turno-socio-title">Registrar turno</h2>
            <div className="consultar-meta">
              <span className="consultar-status consultar-status--info">Socio autenticado</span>
              <span>Reserva personal</span>
            </div>
          </div>
          <button
          ...
          {selectedRango ? (
            <div className="nuevo-turno-modal__summary">
              <div>
                <span>Horario</span>
                <strong>
                  {formatTimeOption(selectedRango.horaDesde)} -{' '}
                  {formatTimeOption(selectedRango.horaHasta)}
                </strong>
              </div>
              <div>
