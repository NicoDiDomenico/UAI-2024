...
async function obtenerSocios() {
    try {
        ...
        socios.forEach(socio => {
            ...
            const fecha = socio.fechaFinActividades ? new Date(socio.fechaFinActividades) : null;
            const fechaFormateada = fecha 
                ? `${fecha.getDate().toString().padStart(2, '0')}/${(fecha.getMonth() + 1).toString().padStart(2, '0')}/${fecha.getFullYear()}`
                : 'No especificada';

            // Determinar el estado CSS
            const estadoCSS = socio.estadoSocio === "Nuevo" ? "active" : "inactive";

            // Añadir un radio button para seleccionar el socio
            socioElemento.innerHTML = `
                <input type="radio" name="socioSeleccionado" value="${socio.idUsuario}" class="select-radio">
                <span class="status ${estadoCSS}"></span>
                <p>Nombre y Apellido: <strong>${socio.nombreApellido || 'No especificado'}</strong></p>
                <p>Estado: <strong>${socio.estadoSocio || 'No especificado'}</strong></p>
