...
namespace MindFit_Intelligence_Backend.Automappers
{
    public class MappingProfile : Profile
    {
        public MappingProfile()
        {
            ...
            // SocioService - Add()
            CreateMap<SocioInsertDto, Usuario>()
                .ForMember(dest => dest.UsuarioGrupos, opt => opt.MapFrom(src =>
                    src.IdGrupos.Select(id => new UsuarioGrupo { IdGrupo = id })));

            // SocioService - Update()
            CreateMap<SocioUpdateDto, Usuario>();

            // ResponsableService - Add()
            CreateMap<ResponsableInsertDto, Usuario>()
                .ForMember(dest => dest.UsuarioGrupos, opt => opt.MapFrom(src =>
                    src.IdGrupos.Select(id => new UsuarioGrupo { IdGrupo = id })));

            // ResponsableService - Update()
            CreateMap<ResponsableUpdateDto, Usuario>();
