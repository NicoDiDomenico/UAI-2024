...
async function obtenerDatosSocio(id) {
    try {
        ...
        if (response.ok) {
            ...
            console.log("Datos obtenidos del socio:", socio);
            ...
            document.querySelector('[name="id"]').value = socio.idUsuario || ''; // idUsuario
            document.querySelector('[name="nombre"]').value = socio.nombreApellido || '';
            document.querySelector(`[name="genero"][value="${socio.genero}"]`).checked = true;
            document.querySelector('[name="dni"]').value = socio.nroDocumento || '';
            ...
            // Manejar los días de asistencia opcionalmente
            const diasAsistencia = socio.diasAsistencia ? socio.diasAsistencia.split(', ') : [];
