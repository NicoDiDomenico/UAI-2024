...
namespace Vista
{
    public partial class frmMenuNegocio : Form
    {
        ...
        private void MostrarReporteEnPantalla()
        {
            // Datos generales
            /*
            var datos = new
            {
                ...
                HorarioLV = $"{nudAperturaLaV.Value:00}:00 - {nudCierreLaV.Value:00}:00",
                HorarioS = $"{nudAperturaS.Value:00}:00 - {nudCierreS.Value:00}:00"
            };
            */
            ...
            var txts = new
            {
                ...
                TELEFONO = txtTelefono.Text,
                CORREO = txtCorreo.Text,
                HORARIOLAV = $"{nudAperturaLaV.Value:00}:00 - {nudCierreLaV.Value:00}:00",
                HORARIOSAB = $"{nudAperturaS.Value:00}:00 - {nudCierreS.Value:00}:00",
                FECHA = DateTime.Now.ToString("dd/MM/yyyy"),
                LOGO = tempLogoPath
            };

            // Lista de socios inactivos
            List<Socio> sociosInactivos = new ControladorGymSocio().ListarSociosInactivos(30);

            // Conteo de estados
            Dictionary<string, int> conteoEstados = new ControladorGymSocio().ContarSociosPorEstado();

            // Abrir el nuevo formulario pasando los datos
            frmReporteNegocio vista = new frmReporteNegocio(txts, sociosInactivos, conteoEstados);
