...
namespace MindFit_Intelligence_Backend.Services
{
    public class UsuarioService : IUsuarioService
    {
        ...
        {
            Cuota? cuotaSocio = await _cuotaRepository.GetUltimaCuotaSocio(idUsuario);
        ...
        public async Task<bool> ValidateDelete(int id)
        {
            ...
            bool tieneCuotaPagada = cuotas.Any(c =>
                c.EstadoCuota == EstadoCuota.Pagado);

            if (tieneCuotaPagada)
                Errors.Add("No se puede eliminar el socio con la cuota a dia");
        ...
        public async Task<bool> ValidateRecover(int idUsuario) { 
            ...
            if (tieneCuotaPagada)
                Errors.Add("El SOcio no se encuentra Eliminado");
