...
select rh_u.*, u.NombreYApellido from RangoHorario_Usuario rh_u
inner join Usuario u
on u.IdUsuario = rh_u.IdUsuario
...
ALTER PROCEDURE [dbo].[SP_REGISTRAR_RANGOHORARIO_USUARIO]
    @IdRangoHorario INT,
    @IdUsuario INT,
    @Mensaje VARCHAR(500) OUTPUT,
    @Resultado BIT OUTPUT
...
BEGIN
    BEGIN TRY
        ...
        -- Validar si ya existe la asignación antes de insertar
        IF EXISTS (
            SELECT 1 
            FROM RangoHorario_Usuario 
            WHERE IdRangoHorario = @IdRangoHorario AND IdUsuario = @IdUsuario
        )
        BEGIN
            SET @Mensaje = 'Ya existe una asignación del entrenador a ese rango horario.';
            SET @Resultado = 0;
            RETURN;
        END
