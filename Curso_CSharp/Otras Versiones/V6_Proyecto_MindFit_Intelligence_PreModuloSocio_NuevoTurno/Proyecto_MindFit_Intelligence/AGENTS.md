...
Frontend authorization must be permission-based.

Permissions are obtained from:

- login response permissions
- form metadata returned by GET /api/Formulario

#### Navigation Buttons

Navigation buttons may be associated with multiple permissions.

A navigation button is visible when the user owns at least one permission associated with that button.
...
Action buttons are associated with a single permission.

An action button is visible only when the user owns the required permission.
...
Do not hardcode permission assignments when they can be obtained from backend metadata.

When available, use:

GET /api/Formulario

to retrieve form-permission relationships.

Backend permission codes are the source of truth.
